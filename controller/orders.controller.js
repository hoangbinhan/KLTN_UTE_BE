...
class OrderServices {
  ...
  static async get(req, res) {
    try {
      ...
      await Object.keys(conditionCustomer).forEach((key) =>
        conditionCustomer[key] === undefined
          ? delete conditionCustomer[key]
          : {}
      );
      await Object.keys(condition).forEach((key) =>
        condition[key] === undefined ? delete condition[key] : {}
      );
      (await condition.dateAdded.$lte)
        ? condition.dateAdded.$lte.setHours(23, 59, 59)
        : delete condition[`dateAdded`];
      Order.aggregate([
        { $match: condition },
        {
          $lookup: {
            ...
            pipeline: [
              {
                $match: { $expr: { $and: [{ $eq: ['$_id', '$$customer'] }] } },
              },
              {
        ...
        for (let i = 0; i < data.length; i++) {
          temp.push({
            ...data[i],
            customerDetail:
              data[i].customerAccountDetail || data[i].customerInfoDetail,
          });
          delete temp[i][`customerAccountDetail`];
          delete temp[i][`customerInfoDetail`];
          if (temp[i].customerDetail.name) {
            let arrLastName = temp[i].customerDetail.name.split(' ');
            arrLastName.shift();
            temp[i].customerDetail.firstName = temp[
              i
            ].customerDetail.name.split(' ')[0];
            temp[i].customerDetail.lastName = arrLastName.join(' ');
          }
        }
        const payload = [...temp].slice(size * page, (page + 1) * size);
  ...
  static async updateStatus(req, res) {
    try {
      ...
      await Order.findOneAndUpdate(
        { _id },
        { status, dateModified: moment().format() },
        { new: true },
        (err, result) => {
          if (err) {
            res.status(400).json({ message: err.message });
