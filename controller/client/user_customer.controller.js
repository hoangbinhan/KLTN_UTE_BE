...
const StaffServices = {
    ...
    register: async (req, res) => {
        try {
            ...
            const newStaff = {
                name, email, password: passwordHash, phoneNumber, address
            }
    ...
    addToCart: async (req,res) =>{
        try{
            ...
            const productInStore = await Product.findOne({_id:product})
            if(!productInStore) return res.status(500).json({msg: 'product invalid'})
            if((productInStore.quantity - quantity) < 0) return res.status(500).json({msg: 'quantity is too large'})
            
            const customer =  await CustomerAccount.findOne({email})
            if(!customer) return res.status(500).json({msg: 'customer invalid'})
            ...
            if(!isAdd){
                customer.cart.push({id: product, quantity:quantity})
            }
            await CustomerAccount.findOneAndUpdate({email}, customer)
            // if(isAdd){
    ...
    login: async (req, res) => {
        try {
            ...
            const customer = await CustomerAccount.findOne({ email })
            if (!customer) return res.status(400).json({ msg: "This email does not exist." })
            const isMatch = await bcrypt.compare(password, customer.password)
            if (!isMatch) return res.status(400).json({ msg: "Password is incorrect." })
            const refresh_token = service.createRefreshToken({ email: customer.email, name: customer.name, role: customer.role })
            const token = service.createAccessToken({ email: customer.email, name: customer.name, role: customer.role })
